...
key = "You_need_RCE_not_me"
app.secret_key = key

@app.route("/robots.txt")
def robots():
    return "flag is in ./qwerty.txt"
...
def index():
    ...
    return render_template_string("""
        ...
        <head>
            ...
            <title>Obtuse Chatbot Revenge 👿</title>
...
def chat():
    ...
    if "flag" in user_input:
        bot_hardcoded_response="Bruh, you're tryin' to flag this, is your config okay?"
        ...
        black_list = [" ","'","'","`","\"","qwerty","os","system"]
        for i in black_list:
            if i in user_input:
                bot_hardcoded_response = "Sorry 👿 black_list = [' ', \"'\", '`', '\"', 'qwerty', 'os', 'system']"
        try:
            bot_response = render_template_string(f"Bot: {bot_hardcoded_response}")
        except Exception:
